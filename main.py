...
def main(args):
    ...
    dataset_train, args.nb_classes = build_dataset(is_train=True, is_test=False, args=args)
    if args.disable_eval:
        args.dist_eval = False
        dataset_val = None
    else:
        dataset_val, _ = build_dataset(is_train=False, is_test=False, args=args)

    if args.test_data_path:
        dataset_test, _ = build_dataset(is_train=False, is_test=True, args=args)
    else:
        dataset_test = None
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            val_stats = evaluate(data_loader_val, model, device, args, use_amp=args.use_amp)
            print(f"F1 score of the model on the {len(dataset_val)} val images: {val_stats['f1'] * 100:.2f}%")
            ...
            print(f'Max f1 score: {max_f1 * 100:.2f}%')
            ...
            if args.model_ema and args.model_ema_eval:
                val_stats_ema = evaluate(data_loader_val, model_ema.ema, device, args, use_amp=args.use_amp)
                print(f"F1 score of the model EMA on {len(dataset_val)} val images: {val_stats_ema['f1'] * 100:.2f}%")
                if max_f1_ema < val_stats_ema["f1"]:
                    max_f1_ema = val_stats_ema["f1"]
                    if args.output_dir and args.save_ckpt:
                        utils.save_model(
                            args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
